structure ArraySequence :> SEQUENCE_EXTRA where type 'a t = 'a ArraySlice.slice =
struct
  ...
  fun tabulate f n =
    if n < 0 then raise Size else
    let
      val a = Primitives.alloc n
    in
      parallelFor (0, n) (fn i => Array.update (a, i, f i));
      AS.full a
    end

  fun tabulateNoGran' n f =
    if n < 0 then raise Size else
    let
      val a = Primitives.alloc n
    in
      parallelForNoGran (0, n) (fn i => Array.update (a, i, f i));
      AS.full a
    end
  ...
  fun iteratePrefixes f b s =
    let val result = AS.full (Primitives.alloc (length s))
  ...
  fun map f s =
    let val (ss, i, l) = ArraySlice.base s
    in if (i = 0)
       then tabulate (fn i => f(Unsafe.Array.sub(ss,i))) (length s)
       else tabulate (fn i => f(nth s i)) (length s)
   end
  ...
  fun unzipWith (f : 'a -> 'b * 'c) s =
    let
      val n = length s
      val (r1, r2) = (Primitives.alloc n, Primitives.alloc n)
    in
  ...
  fun scan (f : 'a * 'a -> 'a) (b : 'a) (s : 'a seq) : 'a seq * 'a =
    let
      val n = length s
      val ps = SeqBasis.scan (G.get ()) f b (0, n) (nth s)
    in
      (AS.slice (ps, 0, SOME n), A.sub (ps, n))
    end
  ...
  struct
    ...
    fun writeMapMerge r cmp (f, g) (s, t) =
      ...
      | (n, m) =>
          ...
          in
            parallelFor (0, length se) (fn i => AS.update (re, i, f (nth se i)));
            parallelFor (0, length te) (fn i => AS.update (re, length se + i, g (nth te i)));
            Primitives.par (fn () => writeMapMerge rl cmp (f, g) (sl, tl),
    ...
    fun mapMerge (cmp : 'a ord) (f : 'a -> 'b, g : 'a -> 'b) (s : 'a seq, t : 'a seq) : 'b seq =
      let val result = AS.full (Primitives.alloc (length s + length t))
    ...
    fun mapScan (f : 'a -> 'b) (g : 'b * 'b -> 'b) (b : 'b) (s : 'a seq) : 'b seq * 'b =
      ...
      then iteratePrefixes (fn (x, y) => g (x, f y)) b s
        ...
        val result : 'b seq = AS.full (Primitives.alloc n)
      in
